...
------------------------------------'''
def output_file(filename, y, sr, ext=""):
    destination = filename[:-4] + ext + '.wav'
...
def noise_remover(file_name):
    ...
    output_file(filename, y_reduced_power, sr, '_pwr')
    output_file(filename, y_reduced_centroid_s, sr, '_ctr_s')
    output_file(filename, y_reduced_centroid_mb, sr, '_ctr_mb')
    output_file(filename, y_reduced_mfcc_up, sr, '_mfcc_up')
    output_file(filename, y_reduced_mfcc_down, sr, '_mfcc_down')
    output_file(filename, y_reduced_median, sr, '_median')
    output_file(filename, y, sr, '_org')