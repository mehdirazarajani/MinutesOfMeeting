...
def improve_agenda_data(title, data):
    ...
    for i, hierarchy in enumerate(agenda_hierarchy_list[:-1]):
        ...
        if len(resultants[agenda_hierarchy_list[i + inc_value]]) == 0:
            ...
            resultants[hierarchy][-1]['ending_page_num'] = resultants[agenda_hierarchy_list[i + inc_value]][0][
                'starting_page_num']
...
if __name__ == '__main__':
    output_folder = "D:\\MinutesOfMeeting\\meeting-transcript-data\\parsed_data"
    output_folder = "D:\\MinutesOfMeeting\\meeting-transcript-data\\parsed_data1"
    for file in os.listdir(folder_path):
        _file = file[:file.index('.')]
        # os.mkdir(output_folder + '\\' + _file)
        # _output_folder = output_folder + '\\' + _file + '\\'
        _output_folder = output_folder + '\\'
        try:
            resultant = read_data_from_file(file)
            ...
            # with open(_output_folder + "data_agenda_{0}.txt".format(_file), 'w') as outfile1:
            #     json.dump(improved_agenda_text, outfile1)

            with open(_output_folder + 'data_meeting_text_pdf_{0}.txt'.format(_file), 'w') as outfile1:
