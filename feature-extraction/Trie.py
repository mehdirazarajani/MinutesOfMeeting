...
def find_all_children(root):
    ...
    for child in node.children:
        if child.word_finished:
            children.add(child.complete_word)
        children.update(find_all_children(child))
    return children


def find_a_word(root, word):
    node = root
    if len(word) == 0:
    ...
    for child in node.children:
        if child.char == word[0]:
            ...
            return find_a_word(child, word[1:])
    if not is_child_present:
        return False